...
export function Card({
  ...
  return (
    ...
    >
      {emoji != null && <View className="text-3xl leading-none drop-shadow-sm">{emoji}</View>}
      {label != null && (
        <View className="text-lg font-bold leading-tight text-center">{label}</View>
      )}
      {sub != null && <View className="text-xs text-inkSoft leading-tight text-center">{sub}</View>}
    </View>
  );
}
