    ...
    if (this.state.hasError) {
      ...
      return (
        <View
          ...
          role="alert"
        >
          <View className="text-7xl animate-floaty">😢</View>
          <Text className="text-2xl font-extrabold text-ink">哎呀，出了点小问题</Text>
          <Text className="text-inkSoft text-base max-w-xs">
            不用着急，点一下下面的按钮就好啦！
          </Text>
          {this.state.error?.message && (
            <Text className="text-xs text-inkSoft/60 bg-white/60 px-3 py-1.5 rounded-2xl max-w-sm break-all">
              {this.state.error.message}
            </Text>
          )}
          <View
