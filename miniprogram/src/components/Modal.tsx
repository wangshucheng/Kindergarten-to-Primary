...
export function Modal({ title, emoji, children, actions, onClose }: ModalProps) {
  ...
  return (
    ...
    >
      ...
      >
        {emoji != null && (
          <View className="text-6xl mb-3 inline-block animate-floaty">{emoji}</View>
        )}
        <Text className="text-2xl font-extrabold text-ink mb-2 tracking-tightish">{title}</Text>
        {children != null && <View className="text-inkSoft mb-5 leading-relaxed">{children}</View>}
