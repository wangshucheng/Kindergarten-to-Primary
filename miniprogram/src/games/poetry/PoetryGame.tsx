...
export function PoetryGame({ config, tts: ttsManager }: GameProps) {
  ...
  return (
    <View className="mx-auto flex w-full max-w-2xl flex-col gap-4 px-4 py-4">
      {/* 顶部：进度 + 标题 + 作者朝代 */}
      <View className="text-center">
        <Text className="text-inkSoft text-sm font-bold">
          第 {index + 1} / {total} 首
        </Text>
        <Text className="text-ink text-4xl font-extrabold">{poem.title}</Text>
        <Text className="text-inkSoft text-base font-bold">
      ...
      {/* 正文卡片 */}
      <View className="bg-cream rounded-3xl p-6 shadow-soft">
        {hideBody ? (
          <Text className="text-inkSoft py-10 text-center text-2xl font-bold">
            🤫 盖住啦，试着背一背
          </Text>
        ) : (
            ...
            {poem.lines.map((line, i) => (
              <View key={i} className="flex items-center gap-3">
                <View className="flex-1">
                  <Text className="text-inkSoft text-sm">{line.pinyin}</Text>
                  <Text className="text-ink text-2xl font-bold">{line.text}</Text>
                </View>
                <Button
                  variant="mint"
                  size="sm"
      ...
      {/* 儿童释义 */}
      {!hideBody && (
        <View className="bg-white/60 rounded-2xl p-5">
          <Text className="text-ink mb-1 text-lg font-bold">💡 儿童释义</Text>
          <Text className="text-ink text-base leading-relaxed">{poem.explanation}</Text>
        </View>
      )}

      {/* 控制条 */}
      <View className="flex flex-wrap items-center justify-center gap-3">
