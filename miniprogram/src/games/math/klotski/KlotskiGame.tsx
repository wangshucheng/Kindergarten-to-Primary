...
export function KlotskiGame({ config, sound, tts: ttsManager, onComplete }: GameProps) {
  ...
  const renderQuestion = () => {
    ...
    if (p.kind === 'sort') {
      return (
        <View className="flex flex-col items-center gap-3 w-full">
          <View className="text-ink font-bold">按从小到大排好顺序，点一点卡片 👇</View>
          ...
          <View className="text-inkSoft text-sm">
    ...
    if (p.kind === 'classify') {
      ...
      return (
        <View className="flex flex-col items-center gap-3 w-full">
          <View className="text-ink font-bold">把卡片分到对应类别（点卡片切换类别）👇</View>
          <View className="flex flex-wrap gap-2 justify-center">
            {p.items.map((it) => (
              <Card
                key={it.id}
    ...
    return (
      <View className="flex flex-col items-center gap-3 w-full">
        <View className="text-ink font-bold">找出规律里缺了的那一个 👇</View>
        <View className="flex flex-wrap gap-2 justify-center">
          {p.items.map((it, i) => (
  ...
  return (
    <View className="flex flex-col items-center gap-3 w-full">
      ...
      {phase === 'question' ? (
        ...
        <>
          <Text className="text-inkSoft text-xs">点方块，把它滑到空格里，把 🚪 送到出口就过关！</Text>
          <View
            className="w-full"
            style={{
              display: 'grid',
          ...
          >
            {board.blocks.map((b) => {
              ...
              return (
                ...
                >
                  <Text className="text-2xl leading-none">{b.label}</Text>
                </View>
              );
            })}
          </View>
