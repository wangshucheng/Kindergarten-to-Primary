...
export function NumberMergeGame({ sound, tts, onComplete, onExit }: GameProps) {
  ...
  return (
    <View className="flex flex-col items-center gap-3" onKeyDown={handleKey} tabIndex={0}>
      <View className="flex items-center justify-between w-full max-w-xs">
        <View className="px-3 py-1.5 rounded-2xl bg-lemon/80 font-bold text-ink">
          目标 {TARGET}
        </View>
        <View className="px-3 py-1.5 rounded-2xl bg-peach/80 font-bold text-ink">
          分数 {state.score}
        </View>
      </View>
      ...
      <Text className="text-inkSoft text-sm text-center">{hint}</Text>
