...
export function GooseCatchGame({ config, sound, tts: ttsManager, onComplete }: GameProps) {
  ...
  const renderTile = useCallback(
    (pos: { row: number; col: number }) => {
      ...
      if (idx >= tiles.length) return <View className="w-full aspect-square rounded-3xl bg-white/40" />;
      ...
      return (
        <Card
          ...
          emoji={tile.emoji}
          label={tile.label}
          sub={tile.sub ? <Text className="text-sm">{tile.sub}</Text> : undefined}
          tone={tile.tone}
  ...
  return (
    <View className="flex flex-col items-center gap-3">
      ...
      {/* 进度条 */}
      <View className="w-full max-w-xs h-3 bg-white/40 rounded-full overflow-hidden">
        <View
      ...
      {/* 目标提示 */}
      {targetTile && (
        <View className="flex items-center gap-2 px-5 py-2 bg-white/60 rounded-3xl shadow-soft">
          <Text className="text-xl">{targetTile.emoji}</Text>
      ...
      {banner && (
        <View className="px-4 py-2 rounded-3xl bg-lemon shadow-soft text-ink font-bold animate-pop">
          {banner}
        </View>
      )}
      ...
      <Text className="text-inkSoft text-sm">
        {subject === 'english'
          ? '找到和目标类别相同的单词，点击它～'
          : '找到和目标拼音相同的字，点击它～'}
      </Text>
