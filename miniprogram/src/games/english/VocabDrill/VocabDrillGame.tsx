...
export function VocabDrillGame({ sound, tts: ttsManager, onComplete }: GameProps) {
  ...
  useEffect(() => {
    if (mode === 'listen') {
      const id = setTimeout(() => tts.speakEn(q.word.en), 250);
  ...
  return (
    <View className="flex flex-col items-center gap-4 w-full max-w-md">
      {/* 模式菜单 */}
      <View className="inline-flex rounded-3xl bg-white/60 p-1 shadow-soft flex-wrap justify-center gap-1">
        {MODES.map((m) => (
          <View
            key={m.key}
      ...
      {/* 题干 */}
      <View className="w-full rounded-4xl bg-cream shadow-soft p-6 flex flex-col items-center gap-3 min-h-[120px] justify-center">
        {mode === 'pick' ? (
          <View
      ...
      <View className="grid grid-cols-2 gap-3 w-full">
        {q.options.map((opt, i) => {
          ...
          const base = 'w-full py-5 rounded-3xl font-bold text-xl text-ink shadow-press flex items-center justify-center text-center';
          const cls = isWrong
