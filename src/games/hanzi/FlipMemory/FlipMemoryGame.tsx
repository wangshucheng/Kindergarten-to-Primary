...
export function FlipMemoryGame({ sound, tts, onComplete }: GameProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <div className="text-center text-ink font-bold">翻开两张一样的字就消除啦！</div>
      <div className="text-inkSoft text-sm">已配对 {matched.size} / {PAIRS}</div>
        ...
        {tiles.map((t) => {
          ...
          return (
            ...
            >
              {isUp ? (
                <>
                  <span className="text-2xl font-extrabold text-ink leading-none">{t.card.char}</span>
                  <span className="text-2xl text-inkSoft">{t.card.pinyin}</span>
                </>
              ) : (
