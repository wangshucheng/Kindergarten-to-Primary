...
export function NumberMinesGame({ config, sound, onComplete }: GameProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      <p className="text-inkSoft text-xs">点格子算出算式答案，避开 💣 翻开所有安全格！</p>
      ...
      {selected && options.length > 0 && (
        <div className="w-full max-w-sm flex flex-col items-center gap-2 rounded-4xl bg-white/70 shadow-soft p-4">
          ...
          {hint && (
            <div className="text-sm text-ink bg-lemon rounded-3xl px-3 py-1 animate-pop">
              小提示：{hint}
            </div>
          )}
        </div>
